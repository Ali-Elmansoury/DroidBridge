...
class TestAppsPageWiring:
    def test_apps_page_replaces_placeholder(self, qtbot):
        from droidbridge.gui.pages.apps import AppsPage

        window = MainWindow()
        qtbot.addWidget(window)

        assert isinstance(window.apps_page, AppsPage)

    def test_apps_page_viewmodels_wired_to_status_and_log(self, qtbot):
        window = MainWindow()
        qtbot.addWidget(window)

        vm = window.apps_page.viewmodels[0]
        vm.statusChanged.emit("Apps test status")

        assert window.status_label.fullText() == "Apps test status"

    def test_apps_page_busy_changed_shows_busy_bar(self, qtbot):
        window = MainWindow()
        qtbot.addWidget(window)
        window.show()
    ...
    def test_apps_tooltip_is_descriptive(self, qtbot):
        from droidbridge.gui.main_window import _SIDEBAR_TOOLTIPS
