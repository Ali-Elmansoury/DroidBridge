...
class BloatwareViewModel(QObject):
    ...
    def disable_app(self, package):
        client, serial = self.context.client, self.context.serial
        fn = functools.partial(apps_ops.disable_app, client, serial, package)
        self._run(fn, lambda _result: self.load_app_info(package))

    def enable_app(self, package):
        client, serial = self.context.client, self.context.serial
        fn = functools.partial(apps_ops.enable_app, client, serial, package)
        self._run(fn, lambda _result: self.load_app_info(package))
