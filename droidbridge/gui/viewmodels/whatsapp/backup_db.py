...
class BackupDbViewModel(QObject):
    ...
    def backup_db(self, app, dest, conflict, verify):
        client, serial = self.context.client, self.context.serial
        self.logMessage.emit("Backing up databases...", "INFO")
        fn = functools.partial(whatsapp_ops.run_backup_db, client, serial, app, dest, conflict, verify)
        self._run(fn, self._on_done, report_progress=True)

    def _on_done(self, result):
        parts = [f"{result['done']}/{result['total']} databases"]
        if result["failed"]:
            parts.append(f"{result['failed']} failed")
        if result["verified"] is True:
            parts.append("verified ✓")
        message = "DB backup complete — " + ", ".join(parts) + "."
