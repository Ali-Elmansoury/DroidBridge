...
class MediaViewModel(QObject):
    ...
    def scan(self, root, before=None):
        client, serial = self.context.client, self.context.serial
        self.logMessage.emit(f"Scanning {root} for media...", "INFO")
        self._run(lambda: storage_ops.get_media(client, serial, root, before=before), self._on_done)

    def _on_done(self, result):
        self.resultChanged.emit(result)
        message = f"Media scan complete — {result['total_count']} file(s), {result['total_size_str']}."
