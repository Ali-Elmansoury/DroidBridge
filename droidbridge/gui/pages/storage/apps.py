...
class AppsPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.top_spin.setToolTip("Number of top apps by size to display.")
        row.addWidget(self.top_spin)
        self.show_all_checkbox = QCheckBox("Show all")
        self.show_all_checkbox.setToolTip("Show every app instead of only the top N.")
        ...
        self.refresh_button.setToolTip("Refresh per-app storage usage with the current filter.")
